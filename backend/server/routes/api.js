...
import { scoreLeadWithAI, determineCategory } from '../services/aiLeadScoring.js';
...
router.post('/ai/score-lead', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: err.message || 'Failed to score lead with AI' });
  }
});
...
router.post('/ai/score-all-leads', authMiddleware, async (req, res) => {
  try {
    const leads = await db.leads.findMany();
    if (leads.length === 0) {
      res.json({ message: 'No leads to score', results: [] });
      return;
    }
    ...
    res.json({ message: 'All leads scored successfully', total: leads.length, scored: results.length, results });
  } catch (err) {
    console.error('Error scoring all leads:', err);
    res.status(500).json({ message: err.message || 'Failed to score all leads' });
  }
});

// Get lead insights for a specific lead
router.get('/ai/lead-insights/:id', authMiddleware, async (req, res) => {
  try {
    ...
    let reasons = [];
    try {
      reasons = lead.aiReasoning ? JSON.parse(lead.aiReasoning) : [];
    } catch {
      reasons = [lead.aiReasoning || 'No detailed reasoning available'];
    }
    ...
    res.status(500).json({ message: 'Failed to fetch lead insights' });
  }
});

// AI Lead Scoring Dashboard Stats
  ...
  try {
    ...
    for (const lead of leads) {
      if (lead.aiScore !== null && lead.aiScore !== undefined) {
        ...
        if (lead.aiCategory === 'Hot') hotCount++;
        else if (lead.aiCategory === 'Warm') warmCount++;
        else if (lead.aiCategory === 'Cold') coldCount++;
      }
    }

    // Sort by aiScore descending for top 5
    scoredLeads.sort((a, b) => b.aiScore - a.aiScore);
    ...
    res.json({
      ...
      warmCount,
      coldCount,
      averageScore: scoredCount > 0 ? Math.round(totalAiScore / scoredCount) : 0,
      top5,
    });
  } catch (err) {
    console.error('Error fetching AI lead scoring stats:', err);
    res.status(500).json({ message: 'Failed to fetch AI lead scoring stats' });
  }
});
